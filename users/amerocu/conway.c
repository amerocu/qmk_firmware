...
unsigned long conway_generation(void){
 ...
 * Game step now runs once per second (1000ms) instead of every scan cycle.
 * */
void conway_step(void)
{
    static uint16_t last_conway_time = 0;
    static bool structures_set = false;

    // Only run game step once per second (1000ms)
    if (timer_elapsed(last_conway_time) < 10) {
        return;  // Not enough time passed, exit early
    }
    last_conway_time = timer_read();  // Update last execution time
